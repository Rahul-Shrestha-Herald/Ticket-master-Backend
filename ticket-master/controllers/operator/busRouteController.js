...
export const addRoute = async (req, res) => {
  try {
    const { bus, from, to, price, pickupPoints, dropPoints, showPrice } = req.body;
...
export const updateRoute = async (req, res) => {
  try {
    ...
    if (showPrice !== undefined) route.showPrice = showPrice;
...
// Update stop fares for dynamic fare calculation
export const updateStopFares = async (req, res) => {
  try {
    const route = await Route.findOne({ _id: req.params.id, operator: req.operator.id });
    if (!route) {
      return res.status(404).json({ success: false, message: 'Route not found.' });
    }
    const { stopFares } = req.body; // Array of { stop: String, fare: Number }
    if (!Array.isArray(stopFares) || stopFares.length < 2) {
      return res.status(400).json({ success: false, message: 'At least 2 stops are required.' });
    }
    route.stopFares = stopFares;
    await route.save();
    res.json({ success: true, message: 'Stop fares updated successfully', route });
  } catch (error) {
    res.status(500).json({ success: false, message: 'Server error. Try again later.' });
  }
};
